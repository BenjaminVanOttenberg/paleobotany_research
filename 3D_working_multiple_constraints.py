...
class Generation:
    def __init__(self, p1, p2, r1, r2, b1, b2, levels):
        self.p1 = p1
        self.p2 = p2
        self.r1 = r1
        self.r2 = r2
    ...
    def adjust_parameters(self):
        self.p1 += random.uniform(-0.05, 0.05)
        self.p2 += random.uniform(-0.05, 0.05)
        self.r1 += 0
        self.r2 += 0
        self.b1 += random.choice([-1, 1]) * random.choice([1, 1, 1, 1, 2, 2, 3, 4, 7])
        self.b2 += random.choice([-1, 1]) * random.choice([1, 1, 1, 1, 2, 2, 3, 4, 7])

    def grow_generation(self, population):
        self.population = population
        self.plants = []
        self.average_maxMoment = 0
        self.average_surfaceAreaToVolume = 0
        if population < 1:
            population = 1
        for i in range(population):
            self.plants.append(Plant(self.p1, self.p2, self.r1, self.r2, self.b1, self.b2, self.levels))
            self.plants[i].generateTelomes()
            self.plants[i].computeMoments()
            self.plants[i].calculateSurfaceAreaToVolume(False)
            self.average_maxMoment += abs(self.plants[i].maxMoment) / population
            self.average_surfaceAreaToVolume += self.plants[i].surfaceAreaToVolume / population
...
def setup_simulation():
    ...
    global levels, p1, p2, r1, r2, b1, b2, left_generation, iterations, max_moments, running, i, surf_area_fitnesses, moment_fitnesses, relative_fitness_scores
    ...
    left_generation = Generation(p1, p2, r1, r2, b1, b2, levels)
    left_generation.grow_generation(population)
...
def run_simulation():
    global left_generation, running, i
    while running:
        i += 1
        ...
        right_generation = Generation(left_generation.p1, 
                                      left_generation.p2, 
                                      left_generation.r1, 
                                      left_generation.r2, 
                                      left_generation.b1,
                                      left_generation.b2,
                                      left_generation.levels)
        right_generation.adjust_parameters()
        right_generation.grow_generation(population)

        # these fitness ratios are percent change, therefore agnostic to magnitude
        # minimize
        if right_generation.average_maxMoment == 0:
            moment_fitness = 1
        else:
            moment_fitness = left_generation.average_maxMoment / right_generation.average_maxMoment 
        # maximize
        surf_area_fitness = right_generation.average_surfaceAreaToVolume / left_generation.average_surfaceAreaToVolume

        fitness_ratios = np.array([moment_fitness, surf_area_fitness])
        fitness_weights = np.array([1, 1])
        weighted_fitness_sum = np.dot(fitness_weights, fitness_ratios)
        # relative to 1
        relative_fitness_score = weighted_fitness_sum / np.sum(fitness_weights)


        # Determine if the right plant is more fit
        if relative_fitness_score > 1:
            left_generation = right_generation
            most_fit_title = "Most Fit Generation (Updated)"
        else:
            most_fit_title = "Most Fit Generation"

        which_plant = random.randint(0, left_generation.population-1)

        # Append data for plotting
        iterations.append(i)
        max_moments.append(left_generation.average_maxMoment)
        
        # Append parameter values for each iteration
        parameter_p1.append(left_generation.p1)
        parameter_p2.append(left_generation.p2)
        parameter_r1.append(left_generation.r1)
        parameter_r2.append(left_generation.r2)
        parameter_b1.append(left_generation.b1)
        parameter_b2.append(left_generation.b2)

        surf_area_fitnesses.append(surf_area_fitness)
        moment_fitnesses.append(moment_fitness)
        relative_fitness_scores.append(relative_fitness_score)

        # Clear and update figure
        fig.clf()
        gs = fig.add_gridspec(4, 2, height_ratios=[3, 1, 1, 1])  # Add extra row for new plot

        # Top-left plot for most fit plant
        ax1 = fig.add_subplot(gs[0, 0])
        left_generation.plants[which_plant].renderPlant2D(ax1, withMoments=True)
        ax1.set_title(most_fit_title)

        # Top-right plot for mutation attempt
        ax2 = fig.add_subplot(gs[0, 1])
        right_generation.plants[which_plant].renderPlant2D(ax2, withMoments=True)
        ...
        ax3 = fig.add_subplot(gs[1, :])
        ax3.plot(iterations, parameter_p1, label="p1")
        ax3.plot(iterations, parameter_p2, label="p2")
        # ax3.set_title("Parameter Values Over Iterations", fontsize=6)
        # ax3.set_xlabel("Iteration", fontsize=6)
        ax3.set_ylabel("Parameter Value", fontsize=6)
        ax3.legend(loc="upper right", fontsize=6)  # Add legend to identify lines
        ...
        #ax4.set_title("Parameter Values Over Iterations", fontsize=6)
        #ax4.set_xlabel("Iteration", fontsize=6)
        ax4.set_ylabel("Parameter Value", fontsize=6)
        ax4.legend(loc="upper right", fontsize=6)  # Add legend to identify lines

        # Bottom plot for tracking parameter changes over iterations
        ax5 = fig.add_subplot(gs[3, :])
        ax5.plot(iterations, surf_area_fitnesses, label="surf_area_fitnesses")
        ax5.plot(iterations, moment_fitnesses, label="moment_fitnesses")
        ax5.plot(iterations, relative_fitness_scores, label="relative_fitness_scores")
        #ax5.set_title("Relative Fitness Scores Normalized to 1", fontsize=6)
        #ax5.set_xlabel("Iteration", fontsize=6)
        ax5.set_ylabel("Relative Fitness Scores", fontsize=6)
        ax5.legend(loc="upper right", fontsize=6)  # Add legend to identify lines
